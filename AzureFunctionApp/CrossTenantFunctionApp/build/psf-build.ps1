...
$requiredModules = (Import-PowerShellDataFile -Path "$workingDirectory/CrossTenantFunctionApp/CrossTenantFunctionApp.psd1").RequiredModules
...
Copy-Item -Path "$workingDirectory/CrossTenantFunctionApp" -Destination "$($buildFolder.FullName)/Modules" -Force -Recurse
$commands = Get-ChildItem -Path "$($buildFolder.FullName)/Modules/CrossTenantFunctionApp/Functions" -Recurse -Filter *.ps1 | ForEach-Object BaseName
Update-PSFModuleManifest -Path "$($buildFolder.FullName)/Modules/CrossTenantFunctionApp/CrossTenantFunctionApp.psd1" -FunctionsToExport $commands
...
foreach ($command in Get-ChildItem -Path "$workingDirectory\CrossTenantFunctionApp\functions\httpTrigger" -Recurse -File -Filter *.ps1) {
...
foreach ($command in Get-ChildItem -Path "$workingDirectory\CrossTenantFunctionApp\functions\eventGridTrigger" -Recurse -File -Filter *.ps1) {
...
foreach ($command in Get-ChildItem -Path "$workingDirectory\CrossTenantFunctionApp\functions\timerTrigger" -Recurse -File -Filter *.ps1) {
